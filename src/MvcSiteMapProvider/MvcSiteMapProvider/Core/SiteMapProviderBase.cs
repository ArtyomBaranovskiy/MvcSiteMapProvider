...
{
	/// <summary>
	/// MvcSiteMapProvider core class
	/// </summary>
	public class SiteMapProviderBase : StaticSiteMapProvider
	{
		#region Private

		private SiteMapProviderBaseConfiguration _configuration;
		...
		protected string[] excludeAssembliesForScan = new string[0];
		protected string[] includeAssembliesForScan = new string[0];
		protected string[] attributesToIgnore = new string[0];
		...
		public override void Initialize(string name, NameValueCollection attributes)
		{
			...
			_configuration = new SiteMapProviderBaseConfiguration(attributes);

			siteMapFile = _configuration.SiteMapFile;
			...
			EnableLocalization = _configuration.EnableLocalization;
			ResourceKey = _configuration.ResourceKey.AsNullIfEmpty() ?? Path.GetFileName(siteMapFile).ToLowerInvariant();
			scanAssembliesForSiteMapNodes = _configuration.ScanAssemblies;
			excludeAssembliesForScan = _configuration.ScanAssembliesExcept;
			includeAssembliesForScan = _configuration.ScanAssembliesOnly;
			attributesToIgnore = _configuration.IgnoredAttributes;
			attributesToInherit = _configuration.InheritedAttributes;
			NodeKeyGenerator = Resolve<INodeKeyGenerator>(_configuration.NodeKeyGenerator);
			ControllerTypeResolver = Resolve<IControllerTypeResolver>(_configuration.ControllerTypeResolver);
			ActionMethodParameterResolver = Resolve<IActionMethodParameterResolver>(_configuration.ActionMethodParameterResolver);
			AclModule = Resolve<IAclModule>(_configuration.AclModule);
			SiteMapNodeUrlResolver = Resolve<ISiteMapNodeUrlResolver>(_configuration.SiteMapNodeUrlResolver);
			SiteMapNodeVisibilityProvider = Resolve<ISiteMapNodeVisibilityProvider>(_configuration.SiteMapNodeVisibilityProvider);
		}
		...
		protected virtual IEnumerable<SiteMapNode> AddDynamicNodesFor(SiteMapNode node, SiteMapNode parentNode)
		{
		...
		protected T Resolve<T>(string typeName) where T : class
		{
			T result = null;
			...
			return result ?? DependencyResolver.Current.GetService<T>();
		}
